...
@Slf4j
public class EmailPasswordAuthenticationProvider extends AbstractAuthenticationProvider {
    ...
    protected void onlySupports(Authentication authentication) {
        Assert.isInstanceOf(EmailPasswordAuthenticationToken.class, authentication, this.messages.getMessage("AbstractAuthenticationProvider.onlySupports", "Only EmailAuthenticationToken is supported"));
    }

    @Override
    public boolean supports(Class<?> authentication) {
        // EmailAuthenticationToken为父类, 通过token类型确定provider，参考ProviderManager.authenticate方法
        return EmailPasswordAuthenticationToken.class.isAssignableFrom(authentication);
    }

    protected Authentication createSuccessAuthentication(Object principal, Authentication authentication, UserDetails user) {
        EmailPasswordAuthenticationToken result = new EmailPasswordAuthenticationToken(principal, authentication.getCredentials(), this.authoritiesMapper.mapAuthorities(user.getAuthorities()));
